...
import type { HealthStatus, SystemStats } from './api.schemas';
...
export const getGetSystemStatsUrl = () => `/api/system/stats`;

export const getSystemStats = async (options?: RequestInit): Promise<SystemStats> =>
  customFetch<SystemStats>(getGetSystemStatsUrl(), { ...options, method: 'GET' });

export const getGetSystemStatsQueryKey = () => [`/api/system/stats`] as const;

export const getGetSystemStatsQueryOptions = <TData = Awaited<ReturnType<typeof getSystemStats>>, TError = ErrorType<unknown>>(
  ...
  const queryFn: QueryFunction<Awaited<ReturnType<typeof getSystemStats>>> = ({ signal }) =>
